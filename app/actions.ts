...
export async function createEmptyInvoice(
  ...
  try {
    ...
    const myToday = dateToPeriode(today);
    ...
    if (user) {
      const newInvoice = await prisma.invoice.create({
        data: {
          id: invoiceId,
          name: clientName + "-" + myToday,
          userId: user?.id,
          issuerName: "Woury Shop",
          issuerAddress: "Bignona",
          clientName: clientName,
          clientAddress: clientAddress,
          invoiceDate: addDaysToDate(new Date(), 0),
          dueDate: addDaysToDate(new Date(), 15),
          vatActive: false,
          vatRate: 0,
        },
      });
